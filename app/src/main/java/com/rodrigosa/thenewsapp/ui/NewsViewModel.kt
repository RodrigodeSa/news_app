...
import com.rodrigosa.thenewsapp.models.Article
import com.rodrigosa.thenewsapp.models.NewsResponse
...
class NewsViewModel(app: Application, private val newsRepository: NewsRepository): AndroidViewModel(app) {
    ...
    private var newSerchQuery: String? = null
    ...
    private fun handleSearchNewsResponse(response: Response<NewsResponse>): Resource<NewsResponse> {
        if (response.isSuccessful) {
            response.body()?.let { resultResponse ->
                if (searchNewsResponse == null || newSerchQuery != oldSearchQuery) {
                    searchNewsPage = 1
                    oldSearchQuery = newSerchQuery
    ...
    private suspend fun headlinesInternet(countryCode: String) {
        ...
        try {
            ...
            when(t) {
                ...
                else -> headlines.postValue(Resource.Error("No Sinal"))
            }
        }
    }

    private suspend fun searchNewsInternet(searchQuery: String) {
        newSerchQuery = searchQuery
